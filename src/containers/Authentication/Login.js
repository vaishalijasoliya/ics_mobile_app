...
import { StyleSheet,View,Text,Dimensions,Image, BackHandler,SafeAreaView,Alert, AsyncStorage, TouchableHighlight, StatusBar } from 'react-native';
      ...
      .then(success => {
        ...
        try {
          AsyncStorage.getItem("usertoken").then(usertoken => {
            console.log('Auth 9', usertoken)
            var userObj = JSON.parse(usertoken);
            if(userObj != null) {
              console.log('Auth 10>> ', userObj)
              if (userObj[0].role == 'user') {
                Actions.userHome()
              }
              else {
                Actions.home();
